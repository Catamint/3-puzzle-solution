...
node* newList(int* firstSquare) {      
    ...
    int* firstpath = blankPath();
    node* firstNode = newNode(firstSquare,firstpath,zeroOf(firstSquare));
    *firstpath=firstNode->zero;
    head->zero=-1; //flag
    ...
    do{
        do{
            ...
            for(int i=0;i<4;i++,neighbour++){
                if (*neighbour == -1) continue; //if no neighbour in this direction
...
int main() {
    int square[TQS]={8,0,1,4,7,2,3,6,5};
    node* answer = NULL;
    if(completed(square)){
